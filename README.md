...
Base modelmeta-llama/Llama-3.2-3B-Instruct (~3.2B params)LoRA target modulesq_proj, v_proj (all 28 layers)Rank r8Alpha α16 (scaling = α/r = 2.0)Trainable params~2.29M (≈ 0.07% of the model)
Task & data
The model is fine-tuned on the SST-2 sentiment dataset, reframed as an instruction-following task. Each example is formatted as:
...
Training setup
TrainerHugging Face TrainerBatch size2 per deviceEpochs3Learning rate3e-4Precisionbf16HardwareNVIDIA RTX 3060 Ti (8 GB)Runtime~14.5 min (final training loss ≈ 1.54)
Setup
Llama-3.2 is a gated model — you'll need a Hugging Face account, acceptance of Meta's license on the model page, and to be logged in:
bashhuggingface-cli login
Dependencies:
bashpip install torch torchvision torchaudio   # CUDA build matching your setup
...
Define LoRALayer and LinearWithLoRA.
Load Llama-3.2-3B-Instruct and freeze all parameters.
Inject LoRA into the attention projections and unfreeze the LoRA params.
Load and format SST-2, then tokenize.
Train with the HF Trainer.
Run inference with classify_sentiment(text).

pythonclassify_sentiment("A genuinely moving, beautifully acted film.")
# -> "positive"
Notes & limitations
